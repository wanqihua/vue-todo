  ...
  <div>
    ...
    <div class="todo_box">
      <div v-for="item in todoList">
        <div class="had_done" :class="{'done':item.done===true}" @click="doneEvent"></div>
        <p class="todo_time">截止时间 {{item.deadline}} </p>
        <p class="todo_detail">{{item.todoItem}}</p>
      </div>
    </div>
  </div>
</template>
